...
# NixOS 环境下的 PATH 设置
PATH=${PATH}:/run/current-system/sw/bin:/usr/bin:/usr/sbin:~/bin
export PATH
#
# ——————————————————————————————————————————————————————————————————————————————————
# __   ___                                    _ _     _
...
# Docker 命令包装器，实际调用 podman
function docker() {
    local args=("$@")
    
    # 处理参数差异
    for i in "${!args[@]}"; do
        # 将 --privileged 转换为 --privileged（podman 也支持）
        # 将 -v 参数中的 /var/run/docker.sock 转换为 /var/run/podman/podman.sock
        if [[ "${args[i]}" == "/var/run/docker.sock" ]]; then
            args[i]="/var/run/podman/podman.sock"
        fi
        # 处理 --net=host，podman 使用 --network=host
        if [[ "${args[i]}" == "--net=host" ]]; then
            args[i]="--network=host"
        fi
    done
...
function touch_chmod() {
    ...
    # 设置所有者（如果可能）
    if [ -n "$RUN_USER" ]; then
        chown "$RUN_USER":"$RUN_USER" "$file_path" 2>/dev/null
    fi
}

# 确保podman policy.json存在
function ensure_podman_policy() {
    # 检查是否已经存在policy.json
    if [ -f "/root/.config/containers/policy.json" ] || [ -f "/etc/containers/policy.json" ]; then
        return 0
    fi
    
    INFO "检测到缺少podman policy.json配置文件，正在创建..."
    
    local policy_dir="/etc/containers"
    ...
    # 创建允许所有镜像的简单策略
    cat > "$policy_file" 2>/dev/null << 'EOF'
{
    "default": [
        {
...
EOF
    ...
    if [ -f "$policy_file" ]; then
        INFO "podman policy.json已成功创建在 ${policy_file}"
    else
        WARN "无法创建podman policy.json，可能会影响镜像拉取"
    fi
}
...
function get_local_ip() {
    # 获取本地 IP 地址，适配 NixOS 环境
    if [[ "${OSNAME}" = "macos" ]]; then
        # macOS
        ifconfig "$(route -n get default | grep interface | awk -F ':' '{print$2}' | awk '{$1=$1};1')" | grep 'inet ' | awk '{print$2}'
    else
        # Linux (包括 NixOS)
...
function oneclick_upgrade_emby() {
    ...
    docker run --rm -v /var/run/podman/podman.sock:/var/run/docker.sock -v /tmp:/tmp ddsderek/runlike "${emby_name}" > "/tmp/container_update_${emby_name}"
...
function emby_close_6908_port() {
    ...
    INFO "获取 ${emby_name} 容器信息中..."
    docker run --rm -v /var/run/podman/podman.sock:/var/run/docker.sock -v /tmp:/tmp ddsderek/runlike -p "${emby_name}" > "/tmp/container_update_${emby_name}"
...
5、其他工具 | Script info: ${DATE_VERSION} OS: ${_os},${OSNAME},${is64bit}
 6、高级配置 | Podman version: ${Blue}${DOCKER_VERSION}${Font} ${IP_CITY}0、退出脚本 | Thanks: ${Sky_Blue}heiheigui,xiaoyaLiu,Harold,AI老G,monlor,Rik${Font}
——————————————————————————————————————————————————————————————————————————————————"
...
function first_init() {
    ...
    *)
    ...
    INFO "检查 Podman 版本"
    DOCKER_VERSION="$(podman -v | sed "s/podman version //g" | cut -d',' -f1)"
